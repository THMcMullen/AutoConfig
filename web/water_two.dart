...
class water_two {
  ...
  water_two(givenGL, passMap, res, givenLocX, givenLocY, givenState) {
    gl = givenGL;
  ...
  createFluidWater(){
    ...
    for (double x = 0.0; x < X; x++) {
      for (double y = 0.0; y < Y; y++) {
        if (blobMap[x.toInt()+1][y.toInt()+1] != 0) {
          wVert.add(y);
    ...
    for (double x = 0.0; x < X; x++) {
      for (double y = 0.0; y < Y; y++) {
        if (blobMap[x.toInt()+1][y.toInt()+1] != 0) {
          wVert[tempCounter] =
              (y * (128 / (X - 1)) + (128 * locX) - (5 * 128));
          tempCounter += 2;

          wVert[tempCounter] =
              (x * (128 / (Y - 1)) + (128 * locY) - (5 * 128));
          tempCounter++;
        }
      }
    }
  ...
  void waterUpdate() {
    ...
    if (noise % 99999 == 1) {
      ...
      if (b[raX * X + raY] == false) {
        if (b[(raX * X) + raY + 1] == false &&
            b[(raX * X) + raY - 1] == false) {
          if (b[((raX + 1) * X) + raY] == false &&
              b[((raX - 1) * X) + raY] == false) {
            h[raX * X + raY] += h[raX * X + raY] < 5.0 ? 2.0 : 0.0;
            h[((raX * X)) + raY + 1] -=
                h[((raX * X)) + raY + 1] < 0.5 ? 0.0 : 0.5;
            h[((raX * X)) + raY - 1] -=
                h[((raX * X)) + raY - 1] < 0.5 ? 0.0 : 0.5;
            h[((raX + 1) * X) + raY] -=
                h[((raX + 1) * X) + raY] < 0.5 ? 0.0 : 0.5;
            h[((raX - 1) * X) + raY] -=
                h[((raX - 1) * X) + raY] < 0.5 ? 0.0 : 0.5;
          }
        }
    ...
    gl.bindBuffer(webgl.RenderingContext.ARRAY_BUFFER, waterVert);
    gl.bufferSubData(webgl.RenderingContext.ARRAY_BUFFER, 0,
        new Float32List.fromList(wVert));
  ...
  void drawWater(Matrix4 viewMat, Matrix4 projectMat) {
    ...
    utils.setMatrixUniforms(gl, viewMat, projectMat, waterUniforms['uPMatrix'],
        waterUniforms['uMVMatrix'], waterUniforms['uNormalMatrix']);

    gl.enableVertexAttribArray(waterAttributes['aVertexPosition']);
    gl.bindBuffer(webgl.ARRAY_BUFFER, waterVert);
    gl.vertexAttribPointer(
        waterAttributes['aVertexPosition'], 3, webgl.FLOAT, false, 0, 0);

    gl.enableVertexAttribArray(waterAttributes['aVertexNormal']);
    gl.bindBuffer(webgl.ARRAY_BUFFER, waterNorm);
    gl.vertexAttribPointer(
        waterAttributes['aVertexNormal'], 3, webgl.FLOAT, false, 0, 0);

    gl.bindBuffer(webgl.ELEMENT_ARRAY_BUFFER, waterInd);

    gl.drawElements(
        webgl.TRIANGLES, waterIndices.length, webgl.UNSIGNED_SHORT, 0);
    //print(waterIndices.length);
    //print("h");
